...
- **Timer-based Rounds**: Fast-paced 30-second rounds with 5-second countdown keep the game exciting
...
- **Realtime**: Socket.io for real-time game synchronization
...
- npm or yarn
- Socket.io server running (see server directory)
...
3. **Set up environment variables**
   ...
   ```
   Fill in your Socket.io server URL and MapTiler API key in the `.env.local` file
...
The game requires a Socket.io server to be running. See the `server` directory for server setup instructions.

The server handles:
- Lobby management and player connections
- Game state synchronization
- Station selection and scoring
